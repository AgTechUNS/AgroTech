...
class RelationalRepository:
    ...
    async def commit_and_refresh(self, session: AsyncSession, obj) -> None:
    ...
    async def create_usuario(
        ...
        async with self._session() as session:
            session.add(usuario)
            await self.commit_and_refresh(session, usuario)
            return usuario

    async def list_usuarios(self) -> list[models.Usuario]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Usuario).order_by(models.Usuario.nombre)
            )
    ...
    async def create_rol(self, nombre_rol: str, descripcion: Optional[str] = None) -> models.Rol:
        ...
        async with self._session() as session:
            session.add(rol)
            await self.commit_and_refresh(session, rol)
            return rol

    async def list_roles(self) -> list[models.Rol]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Rol).order_by(models.Rol.nombre_rol)
            )
            return list(result.scalars().all())
    ...
    async def assign_rol_to_usuario(
        ...
        async with self._session() as session:
            session.add(urc)
            await self.commit_and_refresh(session, urc)
            return urc

    async def list_roles_by_usuario(self, email_usuario: str) -> list[models.UsuarioRolCampo]:
        async with self._session() as session:
            result = await session.execute(
                select(models.UsuarioRolCampo).where(
                    models.UsuarioRolCampo.email_usuario == email_usuario
                )
            )
            return list(result.scalars().all())
    ...
    async def create_campo(
        ...
        campo = models.Campo(
            nombre_campo=nombre,
            coordenadas_campo=coordenadas,
            descripcion_campo=descripcion,
        )
        async with self._session() as session:
            session.add(campo)
            await self.commit_and_refresh(session, campo)
            return campo

    async def list_campos(self) -> list[models.Campo]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Campo).order_by(models.Campo.nombre_campo)
            )
            return list(result.scalars().all())
    ...
    async def get_parcela_by_nombre(self, nombre: str) -> Optional[models.Parcela]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Parcela).where(models.Parcela.nombre_parcela == nombre)
            )
            return result.scalar_one_or_none()
    ...
    async def create_parcela(
        ...
        parcela = models.Parcela(
            ...
            coordenadas_parcela=coordenadas,
            nombre_campo=nombre_campo,
            descripcion_parcela=descripcion,
        )
        async with self._session() as session:
            session.add(parcela)
            await self.commit_and_refresh(session, parcela)
            return parcela
    ...
    async def create_cultivo(self, nombre: str, variedad: Optional[str] = None) -> models.Cultivo:
        ...
        async with self._session() as session:
            session.add(cultivo)
            await self.commit_and_refresh(session, cultivo)
            return cultivo

    async def list_cultivos(self) -> list[models.Cultivo]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Cultivo).order_by(models.Cultivo.nombre_cultivo)
            )
            return list(result.scalars().all())
    ...
    async def get_cultivo_by_nombre(self, nombre: str) -> Optional[models.Cultivo]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Cultivo).where(models.Cultivo.nombre_cultivo == nombre)
            )
            return result.scalar_one_or_none()
    ...
    async def registrar_siembra(
        ...
        async with self._session() as session:
            session.add(rc)
            await self.commit_and_refresh(session, rc)
            return rc
    ...
    async def get_historial_cultivos(
        self, nombre_parcela: str
    ) -> list[models.RegistroCultivo]:
        async with self._session() as session:
            result = await session.execute(
    ...
    async def create_sensor(self, nombre_codigo: str, estado: bool = True) -> models.Sensor:
        ...
        async with self._session() as session:
            session.add(sensor)
            await self.commit_and_refresh(session, sensor)
            return sensor
        ...
        async with self._session() as session:
            result = await session.execute(
                select(models.Sensor).where(
                    models.Sensor.nombre_codigo_sensor == nombre_codigo
                )
            )
            return result.scalar_one_or_none()
    ...
    async def list_sensores(self) -> list[models.Sensor]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Sensor).order_by(models.Sensor.nombre_codigo_sensor)
            )
            return list(result.scalars().all())
    ...
    async def instalar_sensor_en_parcela(
        ...
        async with self._session() as session:
            session.add(sp)
            await self.commit_and_refresh(session, sp)
            return sp
    ...
    async def create_regla(
        ...
        async with self._session() as session:
            session.add(regla)
            await self.commit_and_refresh(session, regla)
            return regla
    ...
    async def list_reglas_by_campo(self, nombre_campo: str) -> list[models.Regla]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Regla)
                .where(models.Regla.nombre_campo == nombre_campo)
                .order_by(models.Regla.nombre_regla)
            )
            return list(result.scalars().all())
    ...
    async def list_reglas(self) -> list[models.Regla]:
        async with self._session() as session:
            result = await session.execute(
                select(models.Regla).order_by(models.Regla.nombre_regla)
            )
            return list(result.scalars().all())
    ...
    async def create_ventana_temporal(
        ...
        async with self._session() as session:
            session.add(vt)
            await self.commit_and_refresh(session, vt)
            return vt
    ...
    async def create_imagen_satelital(
        ...
        async with self._session() as session:
            session.add(img)
            await self.commit_and_refresh(session, img)
            return img
    ...
    async def asociar_imagen_a_parcela(
        ...
        async with self._session() as session:
            session.add(pis)
            await self.commit_and_refresh(session, pis)
            return pis
    ...
    async def create_prediccion(
        ...
        async with self._session() as session:
            session.add(p)
            await self.commit_and_refresh(session, p)
            return p
    ...
    async def list_predicciones_by_campo(
        ...
        async with self._session() as session:
            count_result = await session.execute(
                select(func.count(models.Prediccion.id)).where(
                    models.Prediccion.nombre_campo == nombre_campo
                )
            )
    ...
    async def create_alerta(
        ...
        async with self._session() as session:
            session.add(alerta)
            await self.commit_and_refresh(session, alerta)
            return alerta
    ...
    async def list_alertas_by_usuario(
        ...
        async with self._session() as session:
            count_result = await session.execute(
                select(func.count(models.Alerta.id)).where(
                    models.Alerta.email_usuario == email_usuario
                )
            )
            total = count_result.scalar() or 0
    ...
    async def list_alertas_by_parcela(
        ...
        async with self._session() as session:
            count_result = await session.execute(
                select(func.count(models.Alerta.id)).where(
                    models.Alerta.nombre_parcela == nombre_parcela
                )
            )
            total = count_result.scalar() or 0
    ...
    async def create_ejecucion_batch(
        self, fecha_ini: datetime, estado: str = "EN_CURSO"
    ) -> models.EjecucionBatch:
        eb = models.EjecucionBatch(fecha_ini=fecha_ini, estado=estado)
        async with self._session() as session:
            session.add(eb)
            await self.commit_and_refresh(session, eb)
            return eb
    ...
    async def list_ejecuciones_batch(self) -> list[models.EjecucionBatch]:
        async with self._session() as session:
            result = await session.execute(
                select(models.EjecucionBatch).order_by(
                    models.EjecucionBatch.fecha_ini.desc()
                )
            )
            return list(result.scalars().all())
