"""
token_service.py ÔÇö L├│gica criptogr├ífica de tokens JWT.

Responsabilidades:
  - create_access_token()  : firma y emite el token de sesi├│n corta.
  - create_refresh_token() : firma y emite el token de renovaci├│n.
  - decode_token()         : verifica firma, expiraci├│n y retorna el payload.

Principios de dise├▒o:
  - Stateless: ninguna funci├│n consulta la base de datos.
    Toda la informaci├│n de autorizaci├│n viaja dentro del JWT.
  - Clave secreta: obtenida desde core/config.get_secret_key()
    que resuelve Vault (prod) o .env (dev). Nunca hardcodeada.
  - Algoritmos soportados: HS256 (sim├®trico) y RS256 (asim├®trico),
    seg├║n ALGORITHM configurado en core/config.

Consumidores:
  - auth/service.py            ÔåÆ create_access_token(), create_refresh_token()
  - security/get_current_user  ÔåÆ decode_token()
"""
...
def _build_payload(
    ...
    Campos est├índar JWT:
      sub  : subject ÔÇö email_usuario (PK natural del sistema)
      exp  : expiration ÔÇö validado autom├íticamente por python-jose
      iat  : issued at ÔÇö momento de emisi├│n
...
def create_access_token(user_context: UserContext) -> tuple[str, int]:
    """
    Emite un token JWT de acceso (sesi├│n corta).
...
def create_refresh_token(user_context: UserContext) -> str:
    """
    Emite un token JWT de renovaci├│n (sesi├│n larga).

    Payload m├¡nimo ÔÇö solo sub y expiraci├│n.
    No incluye rol ni campos para minimizar exposici├│n en caso de robo.
...
def decode_token(token: str, expected_type: str = "access") -> UserContext:
    """
    Decodifica y valida un JWT. Retorna el UserContext si es v├ílido.

    Llamado por security/get_current_user.py en cada request protegido.
    No consulta la base de datos ÔÇö validaci├│n 100% stateless.
    ...
    Retorna
    -------
    UserContext con los datos del usuario extra├¡dos del payload.

    Lanza
    -----
    TokenExpiredException  : si el token venci├│ (exp < now).
    InvalidTokenException  : si la firma es inv├ílida, el token est├í
    ...
    # Para refresh tokens el rol no viaja en el payload ÔÇö el caller enriquece
